...
We're not using Django's unit test framework because this is really only one
big test, and Django likes to use an in-memory sqlite database, which can't be
backed up.
"""
import hashlib
...
class Command(BaseCommand):
    ...
    def _handle(self, *args, **options):
        ...
        if not postgres:
            del settings.DATABASES['postgres']
        if not mysql:
            del settings.DATABASES['mysql']
        if not sqlite:
