...
def do(action, database, f, **kwargs):
    ...
    if 'mysql' in engine:
        ...
        raise CommandError('{} in {} engine not implemented'.format(
            'Backup' if action == BACKUP else 'Restore',
            engine
        ))
...
def __postgresql(action, database, f, **kwargs):
    if action == BACKUP:
        ...
        command = [
            PG_RESTORE_BIN, '-{}'.format(kwargs.get('postgres_flags', 'Oxc'))
        ]
    ...
    if 'PASSWORD' in database and database['PASSWORD']:
        # create a pgpass file that always returns the same password, as a
        # secure temp file
