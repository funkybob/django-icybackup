...
def _get_vault_from_arn(arn, settings):
    g = Glacier(aws_access_key_id=settings.AWS_ACCESS_KEY_ID,
...
def reconcile(arn, settings):
    ...
    if not models.GlacierInventory.objects.exclude(
        collected_date__lte=max_collected_date,
        requested_date__lte=max_requested_date
    ).exists():
